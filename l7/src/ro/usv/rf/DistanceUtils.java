...
public class DistanceUtils {
	...
	public static Map<Double, String> calculateEuclidianDistanceMatrice(String[][] learningSet, double[] grades) {
		double[][] mat = new double[learningSet.length][2];

		Map<Double, String> distMap = new HashMap<Double, String>();

		for (int i = 0; i < learningSet.length; i++) {
			double[] c = new double[] { Double.parseDouble(learningSet[i][0]) };
			mat[i][0] = calculateEuclidianDistance(grades, c);

			distMap.put(mat[i][0], learningSet[i][1]);
		}
